        ...
        >
            <div className="mt-2">
                <Link
                    ...
                    // className={`nav-link sticky_li ${activeLink == "/about" ? "active" : ""}`}
                    className={`text-secondary ${activeLink === item?.url ? "active" : ""}`}
                    style={{ textDecoration: 'none', color: 'black', margin: '10px', fontSize: '16px', fontWeight: 'bolder' }}
                >
                    {item.icon && (
                        <FeatherIcon icon={item.icon} className="hori-icon me-1 text-secondary" style={{ width: '16px' }} />
                    )}
                    {item.label}
...
const MainSubmenu = ({ menuItems }: AppMenuProps) => {
    ...
    console.log(activePage)
    const navLinks = document.querySelectorAll('nav a Link').forEach(link => {
        if ((link as HTMLAnchorElement).href.includes(`${activePage}`)) { // No Type Assertion needed
            console.log(`${activePage}`);
            link.classList.add("active");
        }
    });
